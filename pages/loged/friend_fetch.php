<?php
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if(isset($_GET["search"]) ? $_GET["search"]!="" : false )   $friend = mysqli_fetch_assoc(mysqli_query($dbase,"select * from users where $fuid = uid and uname like '%$_GET[search]%'"));
        else                                                        $friend = mysqli_fetch_assoc(mysqli_query($dbase,"select * from users where $fuid = uid "));
        if($friend){
            if($friend["icid"]){
